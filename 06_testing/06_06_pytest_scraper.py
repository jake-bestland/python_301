...
class TestClass:
    ...
    def test_get_valid_html_respoinse(self):
        index_page = get_page_content(self.BASE_URL)
        page = get_page_content(self.url)
        assert index_page.status_code == 200
        assert page.status_code == 200

    def test_get_html_content_returns_html_string(self):
        index_html = get_html_content(self.BASE_URL)
    ...
    def test_get_main_body_gets_main_body_soup(self):
        ...
        assert len(main_body) > 0
        assert "<class 'bs4.BeautifulSoup'>" == str(type(soup))

    def test_get_all_links_gets_links_from_main_body(self):
        html = get_html_content(self.url)
        soup = make_soup(html)
        assert len(get_all_links(soup)) > 0
